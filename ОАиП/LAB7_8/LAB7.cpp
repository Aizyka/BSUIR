...
void Task1(int argc, char** argv) {
    ...
    printf_s("\nMinimum string on pos %d: %s", min_pos, split[min_pos]);
    free(string);
}

void Task2(int argc, char** argv) {
    printf_s("Write string S: ");
    char* S = getstring();
    printf_s("Write string S0: ");
    char* S0 = getstring();
    printf_s("Write k: ");
    int k = getint(0);
    if (k < str_len(S0)) {
        char* sub_s0 = str_sub(S0, str_len(S0) - k, k);
        printf_s("To add: %s", sub_s0);
        str_add(S, sub_s0, str_len(S) / 2);
        printf_s("\nResult: %s", S);
    }
    else
        printf_s("k cant be larger than length of S0 string\n");
    free(S);
    free(S0);
}
